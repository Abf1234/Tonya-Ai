...
The public visual shell and route structure are implemented. Appwrite email/password sign-in, session restoration, account display and current-device sign-out are available once the public Appwrite settings are configured. Verification, chat and reporting screens clearly identify their foundation limitations rather than presenting simulated AI or production records. Protected API calls can opt into the short-lived Appwrite JWT interceptor; backend workflows and evidence uploads will be added in later phases.
